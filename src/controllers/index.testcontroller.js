...
export const loginAuth = async (req, res) => {
    ...
    try{
        const user = await pool.execute(
            "SELECT CORREO_ADMINISTRADOR, CONTRASEÑA_ADMINISTRADOR FROM ADMINISTRADOR WHERE CORREO_ADMINISTRADOR = :EMAIL", [EMAIL] 
        ) 
...
export const postProduct = async (req, res) => {
    ...
    if(ID_ESTADO_PRODUCTO >= 0 && ID_ESTADO_PRODUCTO <= 1 && ID_CATEGORIA >= 0 && ID_CATEGORIA <= 9 && NOMBRE_PRODUCTO.length > 0 && NOMBRE_PRODUCTO.length <= 150 && STOCK > 0 && STOCK <= 9999999999999 && PRECIO > 0 && PRECIO <= 9999999999999){
        ...
        try {
            if(!req.file){
                await pool.execute(`INSERT INTO PRODUCTO VALUES(:ID_PRODUCTO,:ID_ESTADO_PRODUCTO,:ID_CATEGORIA,:NOMBRE_PRODUCTO,:STOCK,:PRECIO,EMPTY_BLOB())`, [lastID, ID_ESTADO_PRODUCTO, ID_CATEGORIA, NOMBRE_PRODUCTO, STOCK, PRECIO])
            }else{
                const IMAGEN = req.file?.buffer;
                await pool.execute(`INSERT INTO PRODUCTO VALUES(:ID_PRODUCTO,:ID_ESTADO_PRODUCTO,:ID_CATEGORIA,:NOMBRE_PRODUCTO,:STOCK,:PRECIO,:IMAGEN)`, [lastID, ID_ESTADO_PRODUCTO, ID_CATEGORIA, NOMBRE_PRODUCTO, STOCK, PRECIO, IMAGEN]) 
            }
        } catch (error) {
...
export const deleteProduct = async (req, res) => {
    ...
    await pool.execute(`DELETE FROM PRODUCTO WHERE ID_PRODUCTO = :ID_PRODUCTO`, [ID_PRODUCTO]);
...
export const deleteMultipleProducts = async (req, res) => {
    ...
    for(let i = 0; i < PRODUCTOS.length; i++)
        sql += (i > 0) ? ", :" + i : ":" + i; 
    sql += ")";
    const result = await pool.execute(sql, PRODUCTOS)
